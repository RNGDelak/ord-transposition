/*
FSS Main functions. Pretty printing is associated with CNF.js via Uniform.js
Limit : Γ₀

for pretty printing use toString()
*/


let version = "1.2";

// Formerly notation.lessOrEqual
function lessOrEqual(a, b) {
    return compareTerms(a, b) <= 0;
}

// Formerly notation.lessThan
function lessThan(a, b) {
    return compareTerms(a, b) === -1;
}

// Formerly notation.equal
function equal(a, b) {
    return compareTerms(a, b) === 0;
}

// Formerly notation.expandLimit
function expandLimit(n) {
    return limit(n + 1);
}

// Formerly notation.isSuccessor
function isSuccessor(array) {
    let s = arrayToString(array);
    return s.length == 0 || s.endsWith("[]");
}

// Renamed from notation.toString to avoid collision with global toString
function arrayToString(array) {
    return JSON.stringify(array).slice(1, -1).replaceAll(/,/g, "");
}

// Formerly notation.fromString
function fromString(s) {
    return JSON.parse("[" + s.replaceAll(/\]\[/g, "],[") + "]");
}

// Formerly notation.convertToNotation
...
function toString(a) {
    ...
    return convertToNotation(arrayToString(a));
}
...
function findPrefixInExpansion(term, parent) {
    if (lessThan(parent, term)) return [null, -1]; // parent should be greater than or equal to term
    let index = 0;
    let element = expand(parent, 0);
    if (lessThan(term, element)) return [null, -1]; // term should be at least parent[0]
    while (true) {
...
function expand(a, n) {
    ...
    let out = [...a];
    let cutNode = out.pop();
    let cutIsSuccessor = isSuccessor(cutNode);
    ...
    if (cutIsSuccessor) {
        ...
        if (equal(root, predecessor)) { // cut node == root + 1
            let zeroth = [...out];
